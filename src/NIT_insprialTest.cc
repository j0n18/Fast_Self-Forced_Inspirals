...
int main(int argc, char *argv[])
{
    ...
    if (model_status == MODEL_LOAD_FAIL)
        exit(0);
...
void interpolate_Fs_and_integrate_NIT_EoM(int mode, double p0, double e0)
{
    ...
    if (mode == NIT_INSPIRAL_DEFAULT)
        sprintf(Ftildes_file, "data/Ftildes_0.dat");
    else
        sprintf(Ftildes_file, "data/Ftildes.dat");
    ...
    {
        cerr << "'Dense_output' setting missing from configuration file." << endl;
        exit(0);
    }
    ...
    { // Dense output
        ...
        {
            cerr << "'n_per_orbit' setting missing from configuration file." << endl;
            exit(0);
        }
    ...
    { // Sparse output
        ...
        while (chi < chi_max)
        {
            ...
            if (fabs(chi_prev / chi - 1.0) < NUM_TOL)
                break;
...
void construct_tilde_Fs()
{
    ...
    while (getline(fv_file, fv_string))
    { // We assume all the Fourier files are on identical grids
...
double getF(double e, double p, double v, vector<Interpolant *> F_interp_cos, vector<Interpolant *> F_interp_sin)
{
    ...
    for (n = 1; n <= n_max; n++)
        F += F_interp_cos[n]->eval(y, e) * cos(n * v) + F_interp_sin[n]->eval(y, e) * sin(n * v);
    return F;
}
...
void compute_waveform(string insp_filename, string out_filename)
{
    ...
    {
        cerr << "'M_solar' setting missing from configuration file." << endl;
        exit(0);
    }
    ...
    {
        cerr << "'Delta_sec' setting missing from configuration file." << endl;
        exit(0);
    }
    ...
    {
        cerr << "'i_max' setting missing from configuration file." << endl;
        exit(0);
    }
...
Complex waveform_h(double p, double e, double v, double phi, double Z, double Phi)
{
    ...
    return ((exp(2. * Complex(0, 1.) * (-phi + Phi)) * XX * Power(1 - Z, 2) + exp(2. * Complex(0, 1.) * (phi - Phi)) * Power(1 + Z, 2) * Conjugate(XX)) * (-2 + p - 2 * e * Cos(v))) /
           ...
           (e * (-1 + Power(Z, 2)) * (2 - p + 2 * e * Cos(v)) * ((Power(e, 2) * (56 - 13 * p) + 4 * (12 - 8 * p + Power(p, 2))) * Cos(v) + e * (52 + 5 * Power(e, 2) - 34 * p + 4 * Power(p, 2) + 2 * (30 + 4 * Power(e, 2) - 7 * p) * Cos(2 * v) - 3 * e * (-8 + p) * Cos(3 * v) + 3 * Power(e, 2) * Cos(4 * v)))) /
               (2. * (-4 * Power(e, 2) + Power(-2 + p, 2)) * Power(p, 2));
}

// Teukolsky Waveform Calculations:
...
Complex ComplexAmp(string insp_filename, string out_filename)
{
    // Check if the assocaited inspiral trajectory file exists
    ifstream insp(insp_filename);
    if (!insp)
    ...
    */
    ...
    // This is where we make y_test (p-2e) and e_test (e) from the inspiral data

    for (int i = 0; i < ps.size() - 1; i++)
    {
        y_test.push_back(ps[i] - 2 * es[i]);
        e_test.push_back(es[i]);
    }
    ...
    // --------------- Calculate the HTeuk values ------------------------------ //

    double m = 2;
    vector<double> w_phi, w_r;

    for (int i = 1; i < ps.size();i++)
    {
        w_phi.push_back( (phis[i]-phis[i - 1])/(ts[i] - ts[i - 1]) );
        w_r.push_back( (vs[i]-vs[i - 1])/(ts[i] - ts[i - 1]) );
    }
    ...
    {
         tt.push_back(ts[i] - U0(ps[i], es[i], vs[i])); // t_interp?
    }

    // loop over all n's and sum with complex exponentials to get HTeuk values:
    Complex sum = 0;
    Complex waveform_T(double Ar,double Ai, double v, double phi,double w_phi, double w_r,double t, double tt, double m, double n)
    {
        Complex power = ( 0 , (m * phi) + (n * v) + (m * w_phi + n * w_r)*(t - tt) ); 
        Complex coeff = (Ar , Ai);
        ...
        {
            for (int j = 0; j < A_re[0].size(); j++)
            {
                sum += waveform_T(A_re[i][j],A_im[i][j],vs[j],phis[j],w_phi[j],w_r[j],ts[j],tt[j],m,i);
            }
        }
    // --------------- Output HTeuk results to file -------------------- //
